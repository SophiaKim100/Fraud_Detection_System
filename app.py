...
#Preprocess information above to put into the function above and predict
distance = haversine(lat, long, merch_lat, merch_long)

#Create button to check for fraud
    ...
    if merchant and category and cc_num:
        ...
        for col in categorical_cols:
            try:
                ...
                input_data[col] = -1
        #Transform the credit card number
        input_data['cc_num'] = input_data['cc_num'].apply(lambda x:hash(x) & (10 ** 2))
        #Make prediction
        prediction = model.predict(input_data)[0]
        result = "Fraudulent Transaction" if prediction == 1 else "Legitamate Transaction"
        st.subheader(f"Prediction : {result}")
    else:
        st.error("Please fill in all fields to check for fraud.")
